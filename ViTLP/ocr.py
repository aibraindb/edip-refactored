...
def greedy_search_continue(encoder_outputs, bboxes, words):
    ...
    for i in range(n - int(n * PREFIX_RATIO), n):
        ...
        pos += 1
    decoder_input_ids = decoder_input_ids[:pos].to(device).unsqueeze(dim=0)
    decoder_input_bboxes = decoder_input_bboxes[:pos, :].to(device).unsqueeze(dim=0)
    flag = False
    i = pos - 1
    decode_ids = []
    word_flag = True
